...
## 1.1 Elevator Pitch
_A privacy-first personal hub with session-level access control that unifies knowledge, tasks, and automations, keeps everything portable, and grows into a local AI assistant without depending on the cloud. Realms act as GTD areas/projects for organization, not security._
...
- **Session-guarded sharing:** Individuals retain absolute control and can safely present data at a chosen session level (e.g., Family) without any higher-level items (Personal/Private/Intimate) surfacing.
...
- **Opinionated on-ramps:** Curated area/project templates and guided setup flows get households productive immediately while showcasing extensibility.
...
4. **Session-level gating:** Support a simple, ordered sensitivity scale and a per-session maximum level to guarantee that higher-sensitivity items never surface when collaborating or presenting.
...
11. **Curated onboarding & templates:** Ship area-specific starter kits, migration helpers, and example automations so new operators see value within the first session while retaining full control to customize or disable them.

## 1.4 Guiding Principles
- **User agency first:** The human remains the final arbiter for every automated action or AI proposal.
- **Security by design:** Default-deny access, strong authentication, and session-level access gating (with item classification) protect household data without relying on realm boundaries.
...
| --- | --- | --- | --- | --- |
| Librarian | Archival, retrieval, curation | Archives, indexes, metadata (R/O except tagging & collections) | Search, summarize, cross-reference | Strict session caps; provenance for changes |
...
- **Privacy-preserving CRM:** Contacts organize by area/workspace and follow item-level sensitivity with session-level gating; exports default to redaction profiles; optional automation suggests relationship updates without disclosing private data beyond the owner’s trust boundary.
